...
аккаунтов (остаток $0.0000, данные на 2026-08), реестр найденных API-граблей.
...
Живая проверка (данные на 2026-08): маркет-мейкер, 42 дня, остаток = $0.00,
day-winrate 64.3%, avg $433/день, maxDD 11.9%.

## Демо (`demos/`, всё через MCP-клиент in-process)

1. `demo1_funding_watch.py` — ватч фандинга: нашёл SHIB +0.147%/1h
   (≈+1284% годовых, лонги платят; catch 2026-08) — готовый алерт.
2. `demo2_trader_check.py` — DD-карта реального маркет-мейкера (2026-08):
   equity $63.5k, PnL $488k, 68 позиций, 100 филлов, maker 21%.
